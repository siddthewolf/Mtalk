...
const SPAWN_Z := -75.0        # how far ahead things appear
...
const GAP_Z := 13.0           # base spacing between obstacle rows
...
func _build_camera() -> void:
	cam = Camera3D.new()
	cam.fov = 72.0
	cam.position = Vector3(0, 3.6, 6.2)
	cam.look_at_from_position(cam.position, Vector3(0, 1.1, -10), Vector3.UP)
	add_child(cam)
...
func _build_player() -> void:
	...
	var hoodie := Color(0.89, 0.37, 0.25)
	torso = _box(Vector3(0.7, 1.0, 0.45), hoodie, player, Vector3(0, 1.05, 0))
	head = _box(Vector3(0.5, 0.5, 0.5), skin, player, Vector3(0, 1.85, 0))
	var hair := _box(Vector3(0.54, 0.18, 0.54), Color(0.22, 0.15, 0.12), player, Vector3(0, 2.05, 0))
	hair.name = "Hair"
	arm_l = _box(Vector3(0.2, 0.7, 0.2), hoodie, player, Vector3(-0.5, 1.15, 0))
	arm_r = _box(Vector3(0.2, 0.7, 0.2), hoodie, player, Vector3(0.5, 1.15, 0))
	leg_l = _box(Vector3(0.24, 0.7, 0.24), Color(0.2, 0.22, 0.28), player, Vector3(-0.2, 0.4, 0))
	leg_r = _box(Vector3(0.24, 0.7, 0.24), Color(0.2, 0.22, 0.28), player, Vector3(0.2, 0.4, 0))
	# Skateboard.
	board = Node3D.new()
	player.add_child(board)
	board.position = Vector3(0, 0.12, 0)
	_box(Vector3(0.95, 0.1, 0.34), Color(0.24, 0.78, 0.86), board, Vector3(0, 0, 0))
	for zx in [-0.32, 0.32]:
		var w := _cyl(0.12, 0.1, Color(0.95, 0.92, 0.85), board, Vector3(zx, -0.08, 0.0))
...
func _add_obstacle(lane: int, type: String) -> void:
	...
	match type:
		"BARRIER":
			var b := _box(Vector3(1.6, 1.1, 0.4), Color(0.95, 0.5, 0.15), n, Vector3(0, 0.7, 0))
			# white hazard stripes
			_box(Vector3(1.62, 0.28, 0.42), Color(0.95, 0.95, 0.96), n, Vector3(0, 0.95, 0))
			_box(Vector3(0.12, 0.7, 0.12), Color(0.3,0.3,0.32), n, Vector3(-0.7, 0.35, 0))
			_box(Vector3(0.12, 0.7, 0.12), Color(0.3,0.3,0.32), n, Vector3(0.7, 0.35, 0))
			b.set_meta("x", 0)
		"OVERHANG":
			# gantry you slide under: posts + high sign with a gap beneath
			_box(Vector3(0.22, 3.0, 0.22), Color(0.5,0.55,0.6), n, Vector3(-0.9, 1.5, 0))
			_box(Vector3(0.22, 3.0, 0.22), Color(0.5,0.55,0.6), n, Vector3(0.9, 1.5, 0))
			_box(Vector3(2.1, 0.9, 0.3), Color(0.18, 0.45, 0.75), n, Vector3(0, 2.7, 0))
		"CAR":
			_make_car(n)
...
func _make_car(parent: Node3D) -> void:
	var col: Color = [Color(0.84,0.2,0.2), Color(0.2,0.4,0.85), Color(0.95,0.8,0.2), Color(0.2,0.7,0.4)][randi() % 4]
	_box(Vector3(1.7, 0.7, 3.6), col, parent, Vector3(0, 0.6, 0))
	_box(Vector3(1.5, 0.6, 1.8), col, parent, Vector3(0, 1.15, -0.2))
	_box(Vector3(1.3, 0.45, 0.1), Color(0.6,0.8,0.95), parent, Vector3(0, 1.15, 0.72))   # rear window
	_box(Vector3(0.35, 0.18, 0.1), Color(1,0.4,0.3), parent, Vector3(-0.55, 0.6, 1.82))  # tail light
	_box(Vector3(0.35, 0.18, 0.1), Color(1,0.4,0.3), parent, Vector3(0.55, 0.6, 1.82))
	for zx in [-0.85, 0.85]:
		for zz in [-1.2, 1.2]:
			var w := _cyl(0.34, 0.25, Color(0.08,0.08,0.09), parent, Vector3(zx, 0.34, zz))
			w.rotation_degrees = Vector3(0, 0, 90)
...
func _animate_player(dt: float) -> void:
	# Lean into turns; swing limbs; crouch when sliding.
	var lean: float = clamp((_lane_x(p_lane) - p_x) * 0.6, -0.5, 0.5)
	player.rotation.z = -lean
	var swing: float = sin(run_cycle) * 0.6
	if sliding:
		torso.position.y = 0.6
		torso.scale = Vector3(1.1, 0.5, 1.2)
		head.position.y = 1.05
		arm_l.rotation.x = 1.2; arm_r.rotation.x = 1.2
		leg_l.rotation.x = 1.2; leg_r.rotation.x = -0.2
	else:
		...
		if not jumping:
			...
			arm_r.rotation.x = swing * 0.7
		else:
			leg_l.rotation.x = 0.5; leg_r.rotation.x = 0.6
			arm_l.rotation.x = -1.0; arm_r.rotation.x = -1.0
